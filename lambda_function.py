...
"""
def validate_data(age, investment_amount, risk, intent_request):
    """
    ...
    if age is not None:
        ...
        if (age <= 0 or age >= 65):
            return build_validation_result(False, "age", 
                "You need to be older than 0 and less than 65 years old")
        
    if investment_amount is not None:
        investment_amount = parse_int(investment_amount)    
        if investment_amount < 5000:
            return build_validation_result(False, "investmentAmount", 
                "You need to be able to invest at least $5000")
                
                
    if risk is not None:
        if risk.lower() not in ['none','low', 'medium', 'high']:
            return build_validation_result(False, "riskLevel", 
                "Risk must be either 'none', 'low', 'medium' or 'high'")
...
def recommend_portfolio(intent_request):
    ...
    risk_map = {
        "none": "100% bonds (AGG), 0% equities (SPY)",
        "low": "60% bonds (AGG), 40% equities (SPY)",
        "medium": "40% High (AGG), 60% equities (SPY)",
        "high": "20% bonds (AGG), 80% equities (SPY)"
    }
    result = ""
    if risk_level.lower() in risk_map:
        result = risk_map[risk_level.lower()]
